        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-white">
          {deals.map((deal, i) => (
            <div key={i} className="flex items-center gap-6 p-8 rounded-[2rem] bg-zinc-900/40 border border-zinc-800/50 hover:border-cyan-500/30 transition-all cursor-pointer group backdrop-blur-sm">
               <div className="p-4 bg-zinc-950 rounded-2xl text-cyan-500 group-hover:scale-110 transition-transform border border-zinc-800">{deal.icon}</div>
               <div>
                 <h4 className="font-black text-lg italic uppercase tracking-wider mb-1">{deal.title}</h4>
