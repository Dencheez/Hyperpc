  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 border-b ${isScrolled ? 'bg-black/95 backdrop-blur-md border-zinc-800 py-4' : 'bg-transparent border-transparent py-7'}`}>
      <div className="container mx-auto px-4 md:px-10 flex justify-between items-center">
        <div onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className="text-3xl font-black tracking-tighter text-white italic cursor-pointer group uppercase">
        ...
        <div className="hidden lg:flex items-center space-x-8 text-white">
          <div className="relative cursor-pointer hover:text-cyan-500 transition-colors" onClick={onOpenCart}>
            <ShoppingCart size={24} />
            <span className="absolute -top-2 -right-2 bg-cyan-600 text-white text-[9px] font-black w-4 h-4 flex items-center justify-center rounded-full shadow-[0_0_10px_rgba(8,145,178,0.5)]">
              {cartCount}
            </span>
          </div>
        </div>
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden absolute top-full left-0 w-full bg-black border-b border-zinc-800 p-6 flex flex-col gap-6 shadow-2xl">
           {navLinks.map((item) => (
            <a key={item.title} href={item.href} className="text-sm font-bold uppercase tracking-widest text-gray-400 hover:text-white" onClick={() => setMobileMenuOpen(false)}>{item.title}</a>
          ))}
